...
class ProductsItemsDAO {
  ...
  static async _validate(productItem: ProductItemData) {
    // Проверка на определенность каждого параметра
    if (
      await (productItem.url === undefined ||
  ...
  static async isExistsIdOneAndIdMany(idOne: number, idMany: number) {
    // Проверка на наличие обоих индексов в таблице
    if (
      (await ProductsItemsRepository.getByIdOneToMany(idOne, idMany)) ===
      undefined
    ) {
      let error = new CustomError(
        "no such exterior_design_id or exterior_design_items_id found",
        404
      );
      throw error;
    }
  ...
  static async getItemsFromProduct(product_id: number) {
    try {
      ...
      const query = await ProductsItemsRepository.getItemsFromProduct(
        product_id
      );
      return query;
    } catch (error) {
