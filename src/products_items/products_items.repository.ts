...
class ProductsItemsRepository {
  static postProductItem(url: string, product_id: number) {
    ...
    return new Promise((resolve, reject) => {
      db.query('INSERT INTO products_items(url, product_id) VALUES ($1, $2) RETURNING *', [url, product_id], (error: any, results: any) => {
        if (error) {
          reject(error);
        } else {
  ...
  static getItemsFromProduct(product_id: number) {
    return new Promise((resolve, reject) => {
      db.query('SELECT * FROM products_items WHERE product_id = $1', [product_id], (error: any, results: any) => {
        if (error) {
          reject(error);
        } else {
